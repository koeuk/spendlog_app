...
/// Create or edit one recurring rule on its own page. Saving runs the rule
...
Future<void> showRecurringForm(
  ...
  return openFormPage<void>(
    context,
    (_) => _RecurringForm(rule: rule, initialKind: kind),
  );
}
...
class _RecurringFormState extends State<_RecurringForm> {
  ...
  Widget build(BuildContext context) {
    ...
    return FormPage(
      title: _editing ? tr('Edit rule') : tr('New recurring rule'),
      formKey: _formKey,
      footer: [
        FilledButton(
...
/// The one confirm dialog both the list's long-press and the page's delete
